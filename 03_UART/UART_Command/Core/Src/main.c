...
void ProcessCommand(char *cmd){
	if (strcmp(cmd, "ON") == 0){
		...
		char *msg = (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_6) == GPIO_PIN_SET) ? "STATUS: ON\r\n" : "STATUS: OFF\r\n";
		HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 100);
	}
	else if (strcmp(cmd, "HELP") == 0){
		const char *list = "Commands: ON, OFF, STATUS, HELP\r\n";
		HAL_UART_Transmit(&huart1, (uint8_t*)list, strlen(list), 100);
	}
}
